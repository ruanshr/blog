...
`reduce`的核心在于降维，降数字`reduce`为一个值，比如求和，而且遍历数组：
...
const flatAllX = (arr) =>
  arr.reduce((acc, v) => (Array.isArray(v) ? acc.concat(flatAllX(v)) : acc.concat(v)), [])

Array.prototype.custFlatAll = function custFlatAll() {
  return this.reduce(
    (acc, v) => (Array.isArray(v) ? acc.concat(v.custFlatAll()) : acc.concat(v)),
    []
  )
}
